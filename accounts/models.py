...
class UserProfile(models.Model):
    ...
    twitter_url = models.URLField(
        max_length=200, blank=True, verbose_name="Twitter 地址"
    )
    linkedin_url = models.URLField(
        max_length=200, blank=True, verbose_name="LinkedIn 地址"
    )
...
class WorkExperience(models.Model):
    ...
    end_date = models.DateField(
        null=True, blank=True, verbose_name="结束日期"
    )  # null=True 表示仍在职
...
class Education(models.Model):
    ...
    degree = models.CharField(
        max_length=50, blank=True, verbose_name="学位"
    )  # 例如：本科, 硕士
