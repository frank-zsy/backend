...
class RedemptionPaymentLineInline(admin.TabularInline):
    """Read-only payment breakdown for a redemption."""

    model = RedemptionPaymentLine
    extra = 0
    readonly_fields = ("point_type", "tag_slug", "amount", "created_at")
    can_delete = False

    def has_add_permission(self, request, obj=None):
        """Payment lines are generated by the redemption service."""
        return False


class ShopItemAllowedTagsInline(admin.TabularInline):
    """Configure the point tags accepted by a shop item."""

    model = ShopItemAllowedTags
    extra = 1
...
class ShopItemAdmin(admin.ModelAdmin):
    ...
    inlines = [ShopItemAllowedTagsInline, RedemptionInline]
...
class RedemptionAdmin(admin.ModelAdmin):
    ...
    @admin.display(description="支付明细")
    def payment_summary(self, obj):
        """Display the point buckets consumed by the redemption."""
        if not obj or not obj.pk:
            return "-"
        lines = []
        for line in obj.payment_lines.all():
            if line.point_type == "cash":
                label = "现金积分"
            elif line.tag_slug:
                label = f"标签积分({line.tag_slug})"
            else:
                label = "通用礼物积分"
