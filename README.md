...
- `docs/testing-strategy.md`：测试分层策略、OWASP 基线、压测与 mutation testing 约定。
...
## 测试与质量
- 全量测试：`just test`
- 安全回归：`just test-security`
- 并发压测：先执行 `just run_plus`，再执行 `just load-test --base-url http://127.0.0.1:8000`
- Mutation Testing：`just mutmut`
